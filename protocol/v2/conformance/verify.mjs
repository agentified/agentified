#!/usr/bin/env node
// Reference verifier for the Ratel catalog-source contract, v2.
//
...
// `If-None-Match` matcher exactly as `protocol/v2/README.md` specifies, then
...
// The frozen v2 content projection: exactly these fields, in this order.
...
// Comma-list tokens are compared by opaque value; the v2 ETags are comma-free
...
function main() {
  ...
  // Frozen v2 wire projection and canonical bytes, including the single
  // representation of an unset searchable description.
  if (!arraysEqual(SKILL_FIELDS, doc.wire?.skillFields)) {
    failures.push(`wire/skillFields: ${JSON.stringify(SKILL_FIELDS)} != ${JSON.stringify(doc.wire?.skillFields)}`);
  }
  for (const v of doc.canonical ?? []) {
    const actual = canonicalSkill(v.skill);
    if (actual !== v.expect) {
      failures.push(`canonical/${v.name}: got ${JSON.stringify(actual)}, expected ${JSON.stringify(v.expect)}`);
    }
  }
  ...
  const n = 1 + (doc.canonical?.length ?? 0) + (doc.etag?.length ?? 0) + (doc.inm?.length ?? 0) + (doc.equalEtags?.length ?? 0) + (doc.distinctEtags?.length ?? 0) + g;
  console.log(`OK — ${n} conformance checks passed (1 wire projection, ${(doc.canonical ?? []).length} canonical, ${doc.etag.length} etag, ${doc.inm.length} if-none-match, ${(doc.equalEtags ?? []).length} equal-groups, ${(doc.distinctEtags ?? []).length} distinct-groups, ${g} intent-graph).`);
}
