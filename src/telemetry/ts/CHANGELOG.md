...
- `SearchTarget.Fact` (`"fact"`) — the third `ratel.search.target` value, beside `tool` and `skill`, for the fact retrieval the SDKs' grounding path emits. Additive: the existing values, their spellings, and `SEMCONV_VERSION` are unchanged, so a host reading `ratel.search.target` keeps working and simply sees a new value once its SDK emits one.
