...
The vocabulary is kept OTel-free so the SDK (emit side), the cloud (read side), and edge/serverless emitters take it weight-free (ADR-0007): importing `@ratel-ai/telemetry` or `ratel_ai_telemetry` pulls no OpenTelemetry SDK. In TypeScript the host owns the OpenTelemetry provider: `@ratel-ai/telemetry` resolves the OTLP config (`resolveOtlpConfig()` reads `RATEL_OTLP_ENDPOINT` and derives a sibling Logs URL, or takes caller-supplied trace and Logs endpoints), and the host builds the exporters and span/log-record processors on the providers it registers, and owns their flush and shutdown. Python keeps turnkey exporter sugar as `init()` behind the optional `[otlp]` extra (`ratel_ai_telemetry.otlp`, reading `RATEL_URL`); the asymmetry is deliberate. The `core/` crate carries the same `ratel.*` constants as the shared source of truth for in-process Rust consumers. The TS and Python helpers' conformance tests build spans and EventRecords from their own constants and assert them against the single shared fixture set in [`conformance/`](conformance/README.md), so those two cannot drift; the `core/` crate stays dependency-free and pins the same constants by literal-equality unit tests.
