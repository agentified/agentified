...
`ratel.tool.execution.details`; experimental changed catalog definitions use
`ratel.catalog.definition` with session-local content-hash deduplication and additionally require
`RATEL_EXPERIMENTAL_CATALOG_DEFINITIONS=true`. All three are off by default. Retrieval experiments emit seven more
