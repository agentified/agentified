...
- Policy keywords are now `Literal` types (`OriginFilterOption`, `ConfirmationOption`, `ProvenanceOption`) rather than `str`, so `origins="baselien"` is a type error rather than a runtime one. Runtime validation is unchanged for callers without a type checker.
- `experimental_enable_adaptive_ranking` accepts the same `origins` / `confirmation` / `provenance` keywords as `experimental_initialize_intent_graph`, so what counts as evidence no longer depends on which path produced the graph. Defaults are unchanged, and the policy survives a `set_trace_sink`.
