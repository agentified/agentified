...
- Policy options are now a closed set of literal types (`OriginFilterOption`, `ConfirmationOption`, `ProvenanceOption`) declared by the SDK rather than the native binding's generated `string` fields, so `{ origins: "baselien" }` is a compile error with completion on the legal values. Runtime validation is unchanged for callers without types.
