...
Use `ToolCatalog` for ranked tools with executable handlers and `SkillCatalog` for ranked playbooks loaded on demand. Expose `searchCapabilitiesTool`, `invokeToolTool`, and `getSkillContentTool` so an agent can discover tools and skills, invoke tools, and load full skill instructions. Tools from existing MCP servers can be ingested into the tool catalog. **Experimental:** the `experimental` namespace adds `experimental.FactCatalog` for constant grounding content — a shop's address, a brand's voice — always-on or retrieval-gated. Two injection modes, mirroring `appendRecall` vs `prepareStep`: `ground()` persists facts into your history and re-injects only when one isn't already fresh in the transcript, while `groundSnapshot()` returns the full per-call set statelessly (nothing stored). This API may change without a major-version bump; import it via `import { experimental } from "@ratel-ai/sdk"`.
