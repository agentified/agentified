...
The line is **the same wire form the `"jsonl"` sink would have written** — field for field, bar the per-record `ts` and `event_id` every envelope-aware sink mints for itself, neither of which replay reads. That is the contract worth relying on: collect lines from every instance into a table, select the ones you want, `join("\n")`, and pass the result straight to `experimentalBuildIntentGraph` — no re-derivation, no wire form of your own.
